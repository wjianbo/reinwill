...
pub fn get_menu_items(
    ...
    menu_items::table.load(conn)
}

pub fn get_technology_by_url_name(
    // 1
    conn: &PgConnection,
    tech_url_name: &str,
) -> Result<Technology, Error> {
    println!("{}", tech_url_name);
...
mod unit_DBテスト {
    ...
    #[test]
    fn get_menu_items関数はMenuの情報をDBから一括で取得する() {
        test_transaction(|conn| {
            let result: Vec<Menu> = get_menu_items(conn).unwrap();
